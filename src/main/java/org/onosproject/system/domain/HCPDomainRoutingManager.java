...
@Service
public class HCPDomainRoutingManager implements HCPDomainRouteService {
...
//        pofController.removeListener(pofSwitchListener);
    ...
    public void setUp() {
        ...
        executor.scheduleAtFixedRate(new hostToEdgeHopTask(),
...
//                log.info("=======after=====topologyVertexList===={}",topologyVertexList.toString());
    ...
    @Override
    public Map<HCPVport, Path> getVportPathByIP(IpAddress ipAddress) {
        if (!ipaddressPathMap.containsKey(ipAddress)) {
            return null;
        }
        return ipaddressPathMap.get(ipAddress);
    }
...
//            log.info("===========DRL message={}======",message);
    ...
    private void processResourceRequest(IPv4Address dstIpv4Address, Set<HCPConfigFlags> flags) {
        ...
        if (flags.contains(HCPConfigFlags.CAPABILITIES_HOP)) {
            Map<HCPVport, Path> pathmap = new HashMap<>();
...
//            log.info("============in the in port here");
            ...
            Path path = null;
            if (connectPoint.deviceId().equals(dstHost.location().deviceId())) {
...
//                log.info("================topology getPath================");
                ...
                if (paths == null) {
                    paths = defaultTopology.getPaths(connectPoint.deviceId(), dstHost.location().deviceId());
                }
                path = (Path) paths.toArray()[0];
            }
